...
import { createResponse, randomPhoneticKey, cache } from '../util';
import { container } from '../container';
import { Router } from 'express';

export const router = Router();

const { prisma, redis } = container;
...
router.post('/urls', hasMatchingAPIKey, async (req, res) => {
	...
	const result = await cache<Url>(`urls/${key}`, await prisma.url.create({ data: { url, slug: key } }));
...
router.delete('/urls/:slug', hasMatchingAPIKey, hasSlugParam, hasURL, async (req, res) => {
	...
	await redis.del(`urls/${req.queriedURL.slug}`);
...
router.put('/urls/:slug', hasMatchingAPIKey, hasSlugParam, hasURL, async (req, res) => {
	const result = await cache<Url>(
		`urls/${req.queriedURL.slug}`,
		await prisma.url.update({ where: { slug: req.queriedURL.slug }, data: { ...req.body } }),
	);
