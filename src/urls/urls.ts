...
router
	.get('/:slug', hasSlugParam, hasURL, async (req, res) => {
		return res.status(301).redirect(req.queriedURL.url);
	})
	.get('/urls/:slug', hasSlugParam, hasURL, async (req, res) => {
		return res.status(200).json(createResponse({ status: 'success', data: { url: req.queriedURL } }));
	})
	.post('/urls', hasMatchingAPIKey, async (req, res) => {
	...
	.delete('/urls/:slug', hasMatchingAPIKey, hasSlugParam, hasURL, async (req, res) => {
	...
	.put('/urls/:slug', hasMatchingAPIKey, hasSlugParam, hasURL, async (req, res) => {
		const result = await prisma.url.update({ where: { slug: req.queriedURL.slug }, data: { ...req.body } });
		
		return res
			.status(200)
			.send(createResponse({ status: 'success', data: { message: 'url updated', from: req.queriedURL, to: result } }));
	});
